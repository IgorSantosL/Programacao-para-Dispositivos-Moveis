...
async function initDatabase() {
  ...
  await ensureColumn('usuarios', 'professor_id', 'INTEGER REFERENCES professores(id) ON DELETE SET NULL');
  await ensureColumn('usuarios', 'aluno_id', 'INTEGER REFERENCES alunos(id) ON DELETE SET NULL');
  ...
  await pool.query(
    `INSERT INTO matriculas_disciplinas (aluno_id, disciplina_id) VALUES ($1, $2)
  ...
  await pool.query(
    `INSERT INTO usuarios (nome, email, login, senha_hash, perfil, professor_id)
     ...
     ON CONFLICT (email) DO UPDATE SET professor_id = EXCLUDED.professor_id`,
  ...
  await pool.query(
    `INSERT INTO usuarios (nome, email, login, senha_hash, perfil, aluno_id)
     ...
     ON CONFLICT (email) DO UPDATE SET aluno_id = EXCLUDED.aluno_id`,
    ['Maria Souza', 'maria.souza@appscholar.com', 'maria.souza@appscholar.com', studentPasswordHash, studentId]
  );
