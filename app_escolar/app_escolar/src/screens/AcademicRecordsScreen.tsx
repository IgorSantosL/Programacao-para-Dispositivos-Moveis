...
export default function AcademicRecordsScreen() {
  ...
  function validateForm() {
    ...
    if (!form.studentId) {
      nextErrors.studentId = 'Selecione um aluno vinculado à disciplina.';
      valid = false;
    }
  ...
  const subjectOptions = useMemo(
    () => subjects.map((item) => ({ label: `${item.nome} • ${item.professor_nome || 'Sem professor'}`, value: String(item.id) })),
    [subjects]
  );

  const filteredStudents = useMemo(() => {
    if (!form.subjectId) return [];
    return students.filter((item) => Array.isArray(item.disciplina_ids) && item.disciplina_ids.includes(Number(form.subjectId)));
  }, [students, form.subjectId]);

  const studentOptions = useMemo(
    () => filteredStudents.map((item) => ({ label: `${item.nome} • ${item.matricula}`, value: String(item.id) })),
    [filteredStudents]
  );
  ...
  return (
    <ScreenContainer>
      <SectionTitle
        ...
        subtitle={`Usuário logado: ${user?.profile === 'admin' ? 'Administrador' : 'Professor'}. O aluno só aparece se estiver vinculado à disciplina selecionada.`}
      />
      ...
      <View style={styles.card}>
        ...
        <SelectField
          label="Aluno"
          placeholder={form.subjectId ? 'Selecione o aluno da disciplina' : 'Selecione uma disciplina primeiro'}
